...
class MavDynamicsINT:
    ...
    def _f(self, state, forces_moments):
        ...
        xyz_forces = np.array([fx,fy,fz]).T
        ...
        # Position Kinematics
        pos_dot = np.array([[e1**2 + e0**2 - e2**2 - e3**2, 2*(e1*e2 - e3*e0), 2*(e1*e3 + e2*e0)],
                            [2*(e1*e2+e3*e0), e2**2 + e0**2 - e1**2 - e3**2, 2*(e2*e3 - e1*e0)],
                            [2*(e1*e3-e2*e0), 2*(e2*e3+e1*e0), e3**2 + e0**2 - e1**2 - e2**2]]) @ np.array([[u],
                                                                                                          [v],
                                                                                                          [w]])
        ...
        # Position Dynamics
        pos_dot = np.array([[r*v - q*w],
                            [p*w - r*u],
                            [q*u - p*v]])
        
        grav_forces = 1/MAV.mass * np.array([[fx],
                                          [fy],
                                          [fz]])
        
        uvw_dot = pos_dot + grav_forces
        
        u_dot = uvw_dot.item(0)
        v_dot = uvw_dot.item(1)
        w_dot = uvw_dot.item(2)

        # rotational kinematics
        e_dot = 0.5 * np.array([[0, -p, -q, -r],
                                [p, 0, r, -q],
                                [q, -r, 0, p],
                                [r, q, -p, 0]]) @ np.array([[e0],
                                                            [e1],
        ...
        # rotatonal dynamics
        pqr_dot = np.array([[MAV.gamma1*p*q - MAV.gamma2*q*r],
        ...
        x_dot = np.array([[pn_dot,pe_dot,pd_dot,u_dot,v_dot,w_dot,e0_dot,e1_dot,e2_dot,e3_dot,p_dot,q_dot,r_dot]]).T
        return x_dot
